...
const Links = ({ query, category, links, totalLinks, linksLimits, linkSkip }) => {
  ...
  const head = () => (
    <Head> 
      <title>{category.name} || {process.env.APP_NAME}</title>
  ...
  const loadPopular = async () => {
    const response = await axios.get(`${process.env.API}/link/popular/${category.slug}`);
  ...
  const listOfPopularLinks = () => (
    popular.map((link, index) => (
      <div className="row alert alert-secondary p-2">
        <div className="col-md-8" onClick={() => handleClickPopular(link._id)}>
